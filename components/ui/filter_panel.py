...
class FilterPanel(ctk.CTkFrame):
    def __init__(self, master, on_filter_change):
        ...
        self.exclude_entry.bind("<Return>", lambda e: self.on_filter_change())

        # Checkbox Promoción
        self.promo_var = ctk.BooleanVar(value=False)
        ctk.CTkCheckBox(
            self, text="Solo Promoción = 1",
            variable=self.promo_var, command=self.on_filter_change
        ...
        ctk.CTkCheckBox(
            ...
            variable=self.desc_dup_var, command=self.on_filter_change
        ).grid(row=4, column=0, columnspan=2, padx=10, pady=5, sticky="w")

        # Botones únicos y duplicados (inicialmente ocultos)
        self.btn_unique = Button(
            self, text="Solo no duplicados", command=lambda: self._set_filter_mode('unique'),
            fg_color="#27ae60", hover_color="#2ecc71",
            row=5, column=0, padx=10, pady=5
        )
        self.btn_dup = Button(
            self, text="Solo duplicados", command=lambda: self._set_filter_mode('dup'),
