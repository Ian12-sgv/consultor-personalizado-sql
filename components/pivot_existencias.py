...
def pivot_existencias(df):
    ...
    # ------------------------------------------------
    # Sólo tomamos una fila por cada 'Concatenar',
    # sin importar cuántos 'CodigoBarra' diferentes haya
    # ------------------------------------------------
    df_fijos = df[CAMPOS_FIJOS].drop_duplicates(subset=['Concatenar'])
    # ------------------------------------------------
    ...
    resultado['Total_Existencia']  = (
        resultado['Casa_matriz_Total'] + resultado['Sucursal_Total']
    )

    # Calcular porcentajes
    resultado['Porcentaje_Existencia_CasaMatriz'] = resultado.apply(
        lambda row: round((row['Casa_matriz_Total'] / row['Total_Existencia']) * 100, 2)
        if row['Total_Existencia'] > 0 else 0,
        axis=1
    )
    resultado['Porcentaje_Existencia_Sucursales'] = resultado.apply(
        lambda row: round((row['Sucursal_Total'] / row['Total_Existencia']) * 100, 2)
        if row['Total_Existencia'] > 0 else 0,
        axis=1
    )

    # Añadir símbolo '%'
    resultado['Porcentaje_Existencia_CasaMatriz'] = (
        resultado['Porcentaje_Existencia_CasaMatriz'].astype(str) + '%'
    )
    resultado['Porcentaje_Existencia_Sucursales'] = (
        resultado['Porcentaje_Existencia_Sucursales'].astype(str) + '%'
    )
    ...
    # ------------------------------------------------
    # Filtrar: eliminar filas donde ambos totales sean 0
    # ------------------------------------------------
    resultado = resultado[
        ~(
            (resultado['Casa_matriz_Total'] == 0) &
            (resultado['Sucursal_Total']    == 0)
        )
    ]
