...
class InicioView(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.chk_promo.grid(row=3, column=0, columnspan=2, padx=10, pady=5, sticky="w")

        # Resaltar duplicados de descuento
        ...
        self.chk_desc_dup.grid(row=4, column=0, columnspan=2, padx=10, pady=5, sticky="w")

        # Botones reactivos para duplicados
        ...
        self.btn_unique.grid(row=5, column=0, padx=10, pady=5, sticky="ew")
        self.btn_dup.grid(row=5, column=1, padx=10, pady=5, sticky="ew")
    ...
    def _update_view(self):
        ...
        exclude_list = [m.strip() for m in exclude_text.split(',') if m.strip()]
    ...
    def _open_pdf_selector(self):
        if self.df_actual is None or self.df_actual.empty:
            ...
            return
        cols = [c for c in self.df_actual.columns if c not in ['Concatenar','Sucursal_Total','Casa_matriz_Total','Total_Existencia','Porcentaje_Existencia_CasaMatriz','Porcentaje_Existencia_Sucursales']]
        if not cols:
        ...
        ctk.CTkButton(btn_frame, text="Cancelar", fg_color="gray", hover_color="darkgray", command=modal.destroy).pack(side='right', padx=(0,5))
        def _confirm():
